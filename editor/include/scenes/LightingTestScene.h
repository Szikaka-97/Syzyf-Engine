...
public:
	void Awake() {
		...
		glBufferData(GL_SHADER_STORAGE_BUFFER, gridSize.x * gridSize.y * gridSize.z * sizeof(glm::uvec2), nullptr, GL_DYNAMIC_DRAW);

		glBindBuffer(GL_SHADER_STORAGE_BUFFER, 0);

		this->cubeMesh = GetScene()->Resources()->Get<Mesh>("./res/models/not_cube.obj");

		ShaderProgram* clusterShader = ShaderProgram::Build()
		.WithVertexShader("./res/shaders/forwardplus/visualiseClusters.vert")
		.WithPixelShader("./res/shaders/forwardplus/visualiseClusters.frag")
		.Link();
	...
	void Update() {
		...
		if (this->engaged) {
			...
			this->clusterMaterial->SetValue("targetCameraInverseProjectionMatrix", glm::inverse(this->savedProjectionMatrix));
			this->clusterMaterial->SetValue("targetCameraProjectionMatrix", this->savedProjectionMatrix);
			this->clusterMaterial->SetValue("targetCameraInverseViewMatrix", glm::inverse(this->savedViewMatrix));
			this->clusterMaterial->SetValue("targetCameraPlanes", this->savedPlanes);

			float spentTime = Time::Current() - this->engagedTime;

			if (spentTime < 3) {
				this->clusterMaterial->SetValue("opacity", (spentTime / 3.0f) / 4.0f);
			}
				...
				GlobalTransform().Position() = glm::mix(startPos, endPos, factor);

				glm::quat startRot = glm::quat(glm::radians(glm::vec3(40, -90.0f, 0)));
				glm::quat endRot = glm::quat(glm::radians(glm::vec3(45, -70.0f, 0)));

				GlobalTransform().Rotation() = glm::slerp(startRot, endRot, factor);
			}
				...
				glm::quat endRot = glm::quat(glm::radians(glm::vec3(40, -90.0f, 0)));
				glm::quat startRot = glm::quat(glm::radians(glm::vec3(45, -70.0f, 0)));
			...
			glCopyNamedBufferSubData(lights->GetLightGridHandle(), this->lightGrid, 0, 0, gridSize.x * gridSize.y * gridSize.z * sizeof(glm::uvec2));

			this->savedViewMatrix = GetScene()->GetGraphics()->GetMainCamera()->ViewMatrix();
			this->savedProjectionMatrix = GetScene()->GetGraphics()->GetMainCamera()->ProjectionMatrix();
			this->savedPlanes = glm::vec2(GetScene()->GetGraphics()->GetMainCamera()->GetNearPlane(), GetScene()->GetGraphics()->GetMainCamera()->GetFarPlane());
		}
	}
	...
	void Render() {
		...
		GetScene()->GetGraphics()->DrawMeshInstanced(this->cubeMesh, 0, this->clusterMaterial, glm::identity<glm::mat4>(), gridSize.x * gridSize.y * gridSize.z, BoundingBox(glm::vec3(-999, -999, -999), glm::vec3(999, 999, 999)));
	}
};
...
inline void InitScene(Scene& mainScene) {
	...
	mainScene.GetComponent<LightSystem>()->SetAmbientLight(glm::vec4(1.0f, 1.0f, 1.0f, 0.1f));
#pragma region World
	...
	auto floorNode = mainScene.resources.Get<GltfScene>("./res/models/rooms/Room T showcase.glb")->Instantiate(&mainScene, nullptr, "floor");
	floorNode->AddObject<Skybox>(skyMat);

	auto monkey = mainScene.resources.Get<GltfScene>("./res/models/big_monkey.glb")->Instantiate(&mainScene, nullptr, "Monkey");
	...
	lightsRoot->GlobalTransform().Rotation() = glm::quat(glm::radians(glm::vec3(40, -90.0f, 0)));
	lightsRoot->AddObject<Light>(Light::DirectionalLight(glm::vec3(1.0, 1.0, 1.0), 1));
...
#pragma endregion
	...
	cameraNode->GlobalTransform().Rotation() = glm::quat(glm::radians(glm::vec3(40, -90.0f, 0)));
