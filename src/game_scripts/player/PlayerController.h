...
public:
    ...
    void TryInitController() {
        ...
        if (!bottleThrower) {
            bottleThrower = GetNode()->AddObject<ThrowBottle>();

            if (this->bottle) {
                if (MeshRenderer* renderer = this->bottle->GetObjectInChildren<MeshRenderer>()) {
                    bottleThrower->SetResources(renderer->GetMesh(), renderer->GetMaterial(0));
                }
...
public:
    ...
    void Update() {
        ...
        float minVelocityLerpFactor = glm::length(this->desiredMovement) < glm::length(movement) ? 0.2f : 0.8f;
        float maxVelocityLerpFactor = glm::length(this->desiredMovement) < glm::length(movement) ? 0.4f : 0.9f;
        ...
        this->targetOffset.y = 0.0f;
        this->aim->SetCrosshairPosition(targetPos);

        if (glm::length(this->targetOffset) > 0.001f) { 
            glm::quat newRotation = glm::quatLookAt(glm::normalize(-targetOffset), glm::vec3(0, 1, 0));

            this->GlobalTransform().Rotation() = newRotation;
            this->virtualController->SetRotation(newRotation);
        }
        ...
        if (throwOomph > 0 && this->throwStrengthCache == 0) {
            ...
            float currentThrowDistance = glm::mix(minThrowDistance, maxThrowDistance, ThrowStrengthEasing(this->throwStrengthAccum));
            glm::vec3 visualHitPoint = targetDir * currentThrowDistance;

            this->aim->crosshair->SetEnabled(true);
            this->aim->SetCrosshairPosition(GlobalTransform().Position().Value() + visualHitPoint);
            ...
            if (this->throwStrengthCache > 0 && this->throwStrengthAccum < 0.7f) {
                if (this->bottleThrower) {
                    glm::vec3 targetDir = glm::length(this->targetOffset) > 0.001f ? glm::normalize(this->targetOffset) : GlobalTransform().Forward();
                    
                    float forwardVelocityBoost = glm::dot(GetStrengthFromVelocity(), targetDir);
                    if (forwardVelocityBoost < 0.0f) forwardVelocityBoost = 0.0f;
                    
                    float finalThrowDist = glm::mix(minThrowDistance, maxThrowDistance, ThrowStrengthEasing(this->throwStrengthCache));
                    glm::vec3 hitPoint = targetDir * (finalThrowDist + forwardVelocityBoost);
                    
                    glm::vec3 finalTarget = this->GlobalTransform().Position().Value() + hitPoint;
                    
                    this->bottleThrower->LaunchBottle(this->throwPoint->GlobalTransform().Position().Value(), finalTarget, this->flightTime);
                }
