...
void EnemyPotato::Update() {
    ...
    // During the airborne attack we move the node directly; do NOT overwrite
    ...
    if (!m_IsAttacking) {
        ...
        // Keep currentPos consistent for FSM distance checks after landing.
    ...
    // Let the attack run to completion before re-entering the normal FSM.
    if (m_IsAttacking) {
