...
namespace Scatter {
    ProjectionFilter::ProjectionFilter(ProjectionSettings settings, JPH::PhysicsSystem* joltSystem, glm::mat4 transform, glm::mat4 inverseTransform)
        : settings(settings), joltSystem(joltSystem), transform(transform), inverseTransform(inverseTransform) {}

    PointStream ProjectionFilter::Process(const PointStream& input) {
        PointStream output;
