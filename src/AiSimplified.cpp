...
void AiSimplified::MoveInDirection(const glm::vec3& direction) {
	...
	void AiSimplified::UpdateStuckDetection(){    if ( m_UsingAStar )
        return; 
    ...
    if (moved < m_MinMovementThreshold) {
        ...
        spdlog::debug("AiNode stuck timer: {:.2f}s (moved {:.3f})", m_StuckTimer, moved);
    } else {
        m_StuckTimer = 0.0f;
    ...
    void AiSimplified::Patrol(){if (!walkPointSet)
		{
			SearchWalkPoint();
		}
		else {
			...
			if (m_PatrolTimeout > 5.0f) {
				spdlog::warn("AiNode: Patrol timeout reached, resetting walk point, current pos ({:.2f},{:.2f},{:.2f}), nex point ({:.2f},{:.2f},{:.2f})", m_TargetPosition.x,m_TargetPosition.y,m_TargetPosition.z, walkPoint.x, walkPoint.y, walkPoint.z);
				walkPointSet = false;
    ...
    void AiSimplified::Chase(){if (m_UsingAStar && !m_Path.empty() && m_CurrentPathIndex < m_Path.size()) {
        ...
        if (m_ChasePathUpdateTimer > 0.5f) {
            ...
            /*if (CanSeePlayer()) {
                ...
                if (!newPath.empty()) {
                    ...
                    m_CurrentPathIndex = 0;
                    spdlog::debug("AiNode: A* path refreshed, {} nodes", m_Path.size());
                } else {
                    spdlog::warn("AiNode: Failed to refresh A* path, reverting to direct chase");
                    m_UsingAStar = false;
                    m_Path.clear();
                }
            }*/
	...
	/*if (!m_InAttackAnimation) {
    	...
    	void AiSimplified::SearchWalkPoint(){if (m_Surface) {
	...
	void AiSimplified::LookForNextPoint(){posIndex++;
		if (posIndex == patrolPoints.size()) {
			posIndex = 0;
		}
		walkPoint = patrolPoints[posIndex];
		walkPointSet = true;}
	     std::vector<glm::vec3> AiSimplified::FindPath(const glm::vec3& start, const glm::vec3& target){if (!m_Surface) return {};
		...
		return {};}
     std::vector<glm::vec3> AiSimplified::GetNeighbors(const glm::vec3& node){if (!m_Surface) return {};
		...
		return neighbors;}
     float AiSimplified::Heuristic(const glm::vec3& a, const glm::vec3& b){glm::vec3 diff = a - b;
		float distance = glm::length(diff);
		return distance;}
     bool AiSimplified::IsWalkable(const glm::vec3& point){if(!m_Surface) return false;
		float groundY = m_Surface->GetGroundHeight(point.x, point.z);
		if (std::abs(point.y - groundY) > 0.5f) return false;
		return true;}
     glm::vec3 AiSimplified::GetNearestWalkable(const glm::vec3& point, float radius ){if (!m_Surface) return point;
		...
		return point;}
	     void AiSimplified::SetPatrolPoints(const std::vector<glm::vec2>& points){for (const auto& point : points) {
